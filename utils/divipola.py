...
# Estructura: { cod_departamento: { "nombre": str, "municipios": { cod_municipio: str } } }
# El código completo de un municipio = cod_dpto + cod_mpio (5 dígitos totales)
# Ej: Bogotá = "11001", Riohacha = "44001", Medellín = "05001"
...
# ─── Conjuntos precalculados para validación O(1) ───────────────────────────
 
# Set con todos los códigos de municipio válidos
CODIGOS_MUNICIPIOS: set[str] = {
    cod
    for depto in DIVIPOLA.values()
    for cod in depto["municipios"]
}
 
# Set con todos los nombres de municipio válidos (en minúsculas para comparación)
NOMBRES_MUNICIPIOS: set[str] = {
    nombre.lower()
    for depto in DIVIPOLA.values()
    for nombre in depto["municipios"].values()
}
 
# ─── Funciones de utilidad ───────────────────────────────────────────────────
 
def es_codigo_valido(codigo: str) -> bool:
    """Verifica si un código DIVIPOLA de 5 dígitos es válido."""
...
def obtener_municipios_por_departamento(cod_dpto: str) -> dict[str, str]:
    """Retorna el diccionario {codigo: nombre} de un departamento."""
    depto = DIVIPOLA.get(cod_dpto)
    if depto:
...
def obtener_departamentos() -> dict[str, str]:
    """Retorna {cod_dpto: nombre_dpto} para poblar el primer desplegable."""
    return {cod: info["nombre"] for cod, info in DIVIPOLA.items()}